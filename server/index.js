...
app.get('/all-resultbacklink', async (req, res) => {
    ...
    try {
        ...
        .populate('user')   // 'user' is ResultbacklinkSchema's field name
        .exec();
        res.send(allResultbacklink);
    } catch (err) {
        res.send(err);
    }
});
...
app.get('/all-resultrelatedsearch', async (req, res) => {
    ...
    try {
        ...
        .populate('user')
        .exec();
        res.send(allResultrelatedsearch);
    } catch (err) {
        res.send(err);
    }
});
...
app.post('/find-userresult', async (req, res) => {
    ...
    try {
        let foundUser = await User.find({username: userName})
        .populate('virtualResultbacklink')
        .populate('virtualResultrelatedsearch').exec();
        res.send(foundUser);
    } catch (err) {
        res.send(err);
    }
